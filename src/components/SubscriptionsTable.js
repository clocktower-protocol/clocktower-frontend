    ...
    for(let i = 0; i < subscriptionArray.length; i++) {
      ...
      if(subscriptionArray[i].status < 1 || isAdmin) {
        ...
        if(role === 1 && Number(totalSubscribers) > 0) {
            ...
            <td key={uuidv4()}><Button type="submit" variant="outline-info" onClick={() => props.setLinkDisplayed(`${process.env.REACT_APP_DOMAIN}/public_subscription/${subscriptionArray[i].subscription.id}/${subscriptionArray[i].subscription.frequency}/${subscriptionArray[i].subscription.dueDay}`)}>Link</Button></td>,
