...
import { SubView } from '../types/subscription';
...
const Calendar: React.FC = () => {
    ...
    const handleMouseEnter = (_info: any) => {
        // Empty handler
    };

    const handleMouseLeave = (_info: any) => {
        // Empty handler
    };
    ...
    const convertToCalEvents = useCallback((subscriptions: SubView[], color: string, isSubscriber: boolean): CalendarEvent[] => {
        let tempEventsArray: CalendarEvent[] = [];
                ...
                switch (frequency) {
                    ...
                    case 0: {
                        ...
                        tempEventsArray.push({
                            title: total + " " + ticker,
                            extendedProps: accountSubscriptions[i].subscription,
                            date: nextEvent.format('YYYY-MM-DD'),
                    ...
                    case 1: {
                        ...
                        tempEventsArray.push({
                            title: total + " " + ticker,
                            extendedProps: accountSubscriptions[i].subscription,
                            date: monthEvent.format('YYYY-MM-DD'),
                            backgroundColor: color
                        });
                            ...
                            for (let d2 = 1; d2 <= maxMonthDays[m2]; d2++) {
                                ...
                                if (counter3 === dueDay) {
                                    ...
                                    tempEventsArray.push({
                                        title: total + " " + ticker,
                                        extendedProps: accountSubscriptions[i].subscription,
                                        date: quarterEvent.format('YYYY-MM-DD'),
                                        backgroundColor: color
                                    });
                                    break;
                                }
                    ...
                    case 3: {
                        ...
                        for (let l = y; l <= (y + 1); l++) {
                            ...
                            tempEventsArray.push({
                                title: total + " " + ticker,
                                extendedProps: accountSubscriptions[i].subscription,
                                date: yearEvent.format('YYYY-MM-DD'),
                                backgroundColor: color
                            });
                        }
                        break;
