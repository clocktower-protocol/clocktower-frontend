...
const SubscribersTable: React.FC<SubscribersTableProps> = (props) => {
    ...
    const totalPages = Math.ceil(subscribersArray.length / itemsPerPage);
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    const currentItems = subscribersArray.slice(startIndex, endIndex);
    ...
    for (let i = 0; i < currentItems.length; i++) {
        ...
        row.push(
            <td key={uuidv4()}>{currentItems[i].accountAddress}</td>,
            <td key={uuidv4()}><Link to={`../admin/history/${currentItems[i].accountAddress}/${false}`}>History</Link></td>,
            <td key={uuidv4()}><Link to={`../admin/subscriptions/${"subscriber"}/${currentItems[i].accountAddress}`}>Subscriptions</Link></td>
        );
            ...
            <Row className="mb-3 mx-2">
                ...
                <Col md={6} className="text-end">
                    <small className="text-muted">
                        Showing {startIndex + 1} to {Math.min(endIndex, subscribersArray.length)} of {subscribersArray.length} entries
                    </small>
                </Col>
            </Row>
