...
const ProvidersTable = (props) => {
    ...
    for(let i = 0; i < providersArray.length; i++) {
        ...
        row.push(
          ...
          <td key={String(providersArray[i])+2}><Link to={`../admin/history/${providersArray[i].accountAddress}/${true}`}>History</Link></td>,
          <td key={String(providersArray[i])+3}><Link to={`../admin/subscriptions/${"provider"}/${providersArray[i].accountAddress}`}>Subscriptions</Link></td>
        )     
